...
class alpsEdAnalyzer : public edm::EDAnalyzer {
 public:

  typedef reco::PFCandidateCollection::const_iterator CI;

  explicit alpsEdAnalyzer(const edm::ParameterSet&);
 ...
 private:
  ...
  float total_,true_,p_,ecal_,hcal_,eta_,phi_,ho_, electron_,muon_,neutral_,charged_,ptt,etaa,phii;
  float etaEcal_,phiEcal_;
  //  double photon_pt,photon_eta,photon_phi;
  int charge_;
  std::vector<float> dr_,Eecal_,Ehcal_,pfcID_,photon_pt,photon_eta,photon_phi,pfc_pt,pfc_eta,pfc_phi,electron_pt,electron_eta,electron_phi,muon_pt,muon_eta,muon_phi, charged_pt, charged_eta,charged_phi, neutral_pt,neutral_eta, neutral_phi;
